...
func TestSyncLightWithBridge(t *testing.T) {
	...
	light := sw.NewLightNode(node2.WithTrustedPeers(addrs[0].String()))
...
func TestSyncStartStopLightWithBridge(t *testing.T) {
	...
	store := node.MockStore(t, node2.DefaultConfig(node2.Light))
	light := sw.NewNodeWithStore(node2.Light, store, node2.WithTrustedPeers(addrs[0].String()))
	...
	require.NoError(t, sw.RemoveNode(light, node2.Light))
...
func TestSyncFullWithBridge(t *testing.T) {
	...
	full := sw.NewFullNode(node2.WithTrustedPeers(addrs[0].String()))
...
func TestSyncLightWithTrustedPeers(t *testing.T) {
	...
	light := sw.NewLightNode(node2.WithTrustedPeers(trustedPeers...))
