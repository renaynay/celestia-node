...
func TestFraudProofBroadcasting(t *testing.T) {
	...
	bridge := sw.NewBridgeNode(node2.WithHeaderConstructFn(header.FraudMaker(t, 10)))
	...
	store := node.MockStore(t, node2.DefaultConfig(node2.Full))
	full := sw.NewNodeWithStore(node2.Full, store, node2.WithTrustedPeers(addrs[0].String()))
	...
	require.NoError(t, sw.RemoveNode(full, node2.Full))
