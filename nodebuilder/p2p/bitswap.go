...
func bsFromDatastore(ctx context.Context, ds datastore.Batching) (blockstore.Blockstore, error) {
...
func bsFromEDSStore(ctx context.Context, store *eds.Store) (blockstore.Blockstore, error) {
	bs, err := blockstore.CachedBlockstore(
		ctx,
		store.Blockstore(),
		blockstore.CacheOpts{
