...
func DefaultConfig(tp node.Type) Config {
	...
	switch tp {
	case node.Light:
		cfg.SampleTimeout = modp2p.DefaultBlockTime * time.Duration(cfg.ConcurrencyLimit)
	case node.Full:
		...
		// scenario
		cfg.SampleTimeout = 2 * modp2p.DefaultBlockTime * time.Duration(cfg.ConcurrencyLimit)
	}
	return Config(cfg)
}
